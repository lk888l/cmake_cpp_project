...
**中文** | [English](api-reference_en.md)
...
`delay_ms` 必须由平台提供。Linux 使用 `sleep_for`，STM32 可使用 `HAL_Delay`，ESP32 可使用 RTOS 延时包装。当前接口只表达毫秒延时；微秒精度限制见 [mcu-porting_cn.md](mcu-porting_cn.md)。
