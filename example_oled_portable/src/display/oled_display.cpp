...
{
    switch (rotation) {
    case Rotation::deg0: return U8G2_R0;
    case Rotation::deg90: return U8G2_R1;
    case Rotation::deg180: return U8G2_R2;
    case Rotation::deg270: return U8G2_R3;
...
{
    switch (status) {
    case DisplayStatus::ok: return "ok";
    case DisplayStatus::invalid_argument: return "invalid_argument";
    case DisplayStatus::not_initialized: return "not_initialized";
    case DisplayStatus::buffer_overflow: return "buffer_overflow";
    case DisplayStatus::transport_error: return "transport_error";
...
{
    if (config_.controller == Controller::ssd1315) {
...
{
    if (config_.delay_ms == nullptr) { return DisplayStatus::invalid_argument; }
    resetStatus();
    u8g2_InitDisplay(&u8g2_);
    if (last_status_ != DisplayStatus::ok) { return last_status_; }
...
{
    if (!initialized_) { return DisplayStatus::not_initialized; }
    resetStatus();
    u8g2_SendBuffer(&u8g2_);
    return last_status_;
}
...
{
    return static_cast<std::size_t>(u8g2_GetBufferTileHeight(const_cast<u8g2_t*>(&u8g2_))) *
           static_cast<std::size_t>(u8g2_GetBufferTileWidth(const_cast<u8g2_t*>(&u8g2_))) * 8U;
}

std::uint8_t OledDisplay::byteCallback(u8x8_t* u8x8, std::uint8_t msg,
                                       std::uint8_t arg_int, void* arg_ptr)
{
    auto* self = static_cast<OledDisplay*>(u8x8_GetUserPtr(u8x8));
    return self == nullptr ? 0 : self->onByteMessage(msg, arg_int, arg_ptr);
}

std::uint8_t OledDisplay::gpioDelayCallback(u8x8_t* u8x8, std::uint8_t msg,
                                            std::uint8_t arg_int, void*)
{
    auto* self = static_cast<OledDisplay*>(u8x8_GetUserPtr(u8x8));
    return self == nullptr ? 0 : self->onDelayMessage(msg, arg_int);
}

std::uint8_t OledDisplay::onByteMessage(std::uint8_t msg, std::uint8_t count, const void* data)
{
    switch (msg) {
    ...
    case U8X8_MSG_BYTE_END_TRANSFER:
        if (transfer_size_ == 0) { return 1; }
        if (device_.write(transfer_buffer_.data(), transfer_size_) != bsp::I2CStatus::ok) {
...
std::uint8_t OledDisplay::onDelayMessage(std::uint8_t msg, std::uint8_t value)
{
    switch (msg) {
