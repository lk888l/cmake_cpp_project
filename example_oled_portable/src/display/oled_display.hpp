...
enum class Controller : std::uint8_t { ssd1306, ssd1315 };
...
private:
    static std::uint8_t byteCallback(u8x8_t* u8x8, std::uint8_t msg,
                                     std::uint8_t arg_int, void* arg_ptr);
    static std::uint8_t gpioDelayCallback(u8x8_t* u8x8, std::uint8_t msg,
                                          std::uint8_t arg_int, void* arg_ptr);

    std::uint8_t onByteMessage(std::uint8_t msg, std::uint8_t count, const void* data);
    std::uint8_t onDelayMessage(std::uint8_t msg, std::uint8_t value);
    void resetStatus();
