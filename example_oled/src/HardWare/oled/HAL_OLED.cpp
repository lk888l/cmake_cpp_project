...
#include <vector>
#include <memory>
#include "Peripheral/i2c/i2c_hard.hpp"
/**
  * 数据存储格式：
...
static std::unique_ptr<I2C_Hard> i2c = nullptr;
/*********************全局变量*/
...
{
	i2c = std::make_unique<I2C_Hard>("/dev/i2c-3", 0x3C);	//创建I2C_Hard对象，打开I2C总线并设置从机地址
}
...
{
	// 构造 SSD1306 命令包：控制字节 0x00 + 命令字节
    uint8_t buf[2] = {0x00, Command};
    // 使用 I2C 类的连续写方法（假设名为 writeData 或类似）
    // 不要手动写 Start/Stop/SendByte(0x78)
    i2c->writeBytes(buf, 2);
}
...
{
// 构造 SSD1306 数据包：控制字节 0x40 + 多个数据字节
	std::vector<uint8_t> dataVec;
	dataVec.resize(Count + 1);
	dataVec[0] = 0x40; // 控制字节，表示后面是数据
	memcpy(&dataVec[1], Data, Count);
    i2c->writeBytes(dataVec.data(), Count + 1);
}
