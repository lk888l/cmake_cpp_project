...
St7789::~St7789()
{
    (void)backlight_.set(false);
}

bsp::Status St7789::command(std::uint8_t value)
{
    const bsp::Status status = dc_.set(false);
    return status == bsp::Status::ok ? spi_.write(&value, 1) : status;
}

bsp::Status St7789::data(const std::uint8_t* bytes, std::size_t length)
{
    const bsp::Status status = dc_.set(true);
    return status == bsp::Status::ok ? spi_.write(bytes, length) : status;
}

bsp::Status St7789::commandData(std::uint8_t command_value,
                                const std::uint8_t* bytes,
                                std::size_t length)
{
    const bsp::Status status = command(command_value);
...
bsp::Status St7789::init()
{
    ...
    (void)backlight_.set(false);
    (void)reset_.set(true);
    std::this_thread::sleep_for(10ms);
    (void)reset_.set(false);
    ...
    std::uint8_t madctl = config_.bgr ? 0x08 : 0x00;
    ...
    const std::array<std::uint8_t, 14> gamma_pos = {
        0xD0, 0x04, 0x0D, 0x11, 0x13, 0x2B, 0x3F, 0x54, 0x4C, 0x18, 0x0D, 0x0B, 0x1F, 0x23};
    const std::array<std::uint8_t, 14> gamma_neg = {
    ...
    for (const auto& [address, value] : registers) {
        if ((status = commandData(address, &value, 1)) != bsp::Status::ok) return status;
    }
...
bsp::Status St7789::setAddressWindow(std::uint16_t x1, std::uint16_t y1,
                                     std::uint16_t x2, std::uint16_t y2)
{
    x1 = static_cast<std::uint16_t>(x1 + config_.x_offset);
    x2 = static_cast<std::uint16_t>(x2 + config_.x_offset);
    y1 = static_cast<std::uint16_t>(y1 + config_.y_offset);
    y2 = static_cast<std::uint16_t>(y2 + config_.y_offset);
...
bsp::Status St7789::writePixelBytes(const std::uint8_t* bytes, std::size_t length)
{
