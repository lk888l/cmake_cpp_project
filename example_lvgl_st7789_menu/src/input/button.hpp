...
[[nodiscard]] constexpr bool isPressedLevel(const ButtonConfig& config,
                                            bool physical_level_high) noexcept
{
...
/// Linux GPIO v2 manager. Callbacks run serially on the GPIO event thread.
/// A callback must remain short and must never call LVGL directly.
class ButtonManager {
public:
    ...
    /// Empty while no worker-thread failure has occurred. Startup failures are
    /// still reported synchronously by start().
