...
{
    const bool physical_level_high = event_id == GPIO_V2_LINE_EVENT_RISING_EDGE;
...
public:
        ...
        : configs_(std::move(configs))
    {
        ...
        for (const auto& config : configs_) {
            if (config.id.empty() || config.chip_path.empty() || !ids.insert(config.id).second) {
                throw std::invalid_argument(
                    "button ids and chip paths must be non-empty; ids must be unique");
            }
            machines_.push_back(std::make_unique<detail::ButtonStateMachine>(config));
        }
    }
    ...
    void start()
    {
        ...
        if (running_.load(std::memory_order_acquire) || worker_.joinable() || epoll_fd_ >= 0) {
            throw std::logic_error("ButtonManager is already active; call stop before restarting");
        }
    ...
    void stop()
    {
        ...
        if (was_running && stop_fd_ >= 0) {
            ...
            if (::write(stop_fd_, &signal, sizeof(signal)) < 0 && errno != EAGAIN) {
                std::cerr << "GPIO manager wakeup failed: " << std::strerror(errno) << '\n';
            }
        }
...
private:
    ...
    void requestLines()
    {
        ...
        for (std::size_t index = 0; index < configs_.size(); ++index) {
            ...
            device.chip_fd = ::open(config.chip_path.c_str(), O_RDONLY | O_CLOEXEC);
            if (device.chip_fd < 0) {
            ...
            std::strncpy(request.consumer, "example-gpio-button", sizeof(request.consumer) - 1);
            ...
            values.mask = 1;
            if (::ioctl(device.line_fd, GPIO_V2_LINE_GET_VALUES_IOCTL, &values) < 0) {
            ...
            addFd(device.line_fd, kButtonTagBase + static_cast<std::uint32_t>(index));
        }
    }
    ...
    void run() noexcept
    {
        try {
            while (running_.load(std::memory_order_acquire)) {
                ...
                // Kernel-timestamped GPIO edges take precedence at exact timer boundaries.
                for (int i = 0; i < count; ++i) {
                    const auto tag = ready[static_cast<std::size_t>(i)].data.u32;
            ...
            running_.store(false, std::memory_order_release);
            std::cerr << "GPIO event loop stopped: " << error.what() << '\n';
        }
    }
    ...
    void readEdges(std::size_t index)
    {
        ...
        for (;;) {
            const ssize_t bytes = ::read(devices_[index].line_fd, events.data(), sizeof(events));
            if (bytes < 0) {
            ...
            if (bytes == 0) {
                throw std::runtime_error("GPIO line event fd closed unexpectedly");
            }
            if (bytes % static_cast<ssize_t>(sizeof(gpio_v2_line_event)) != 0) {
                throw std::runtime_error("short GPIO v2 line event record");
            }

            const auto count = static_cast<std::size_t>(bytes) / sizeof(gpio_v2_line_event);
    ...
    void armTimer()
    {
        ...
        if (::timerfd_settime(timer_fd_, TFD_TIMER_ABSTIME, &timer, nullptr) < 0) {
...
void ButtonManager::setCallback(ButtonCallback callback) { impl_->setCallback(std::move(callback)); }
void ButtonManager::start() { impl_->start(); }
void ButtonManager::stop() { impl_->stop(); }
bool ButtonManager::isRunning() const noexcept { return impl_->isRunning(); }
